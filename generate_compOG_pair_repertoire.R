...
queryorthodbpairs <- ggplot(arrange(query_heatmap_df_sym,score), aes(x = factor(OrthoDB1, ordered=T, levels=as.factor(unique(arrange(query_heatmap_df_sym,score)$OrthoDB1))), y = factor(OrthoDB2, ordered=T, levels=as.factor(unique(arrange(query_heatmap_df_sym,score)$OrthoDB2))), fill = score)) +
  geom_tile() +
  scale_fill_viridis_c() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 4.5)) +
  theme(axis.text.y = element_text(size = 4.5)) +
  labs(title = "Pairwise OrthoDB Difference (Sum of Squares)", x = "CompOG partner 1", y = "CompOG partner 2")
  
ggsave(file= paste0("Results/", filename,"pairs_plot.svg"), plot=queryorthodbpairs, width=20, height=20)
